...
func (k Keeper) TransmitIbcCreatePoolPacket(
	...
	// AK-JFT
	k.Logger(ctx).Info(fmt.Sprintf("Func=TransmitIbcCreatePoolPacket|SourcePort=%s|SourceChannel=%s|Weights=%s|InitialDeposits=%s|SwapFee=%s|ExitFee=%s|FutureGovernor=%s",
		sourcePort, sourceChannel, packetData.Weights, packetData.InitialDeposit, packetData.SwapFee, packetData.ExitFee, packetData.FutureGovernor))

	sourceChannelEnd, found := k.ChannelKeeper.GetChannel(ctx, sourcePort, sourceChannel)
	//sourceChannelEnd, found := k.GetChannel(ctx, sourcePort, sourceChannel)
	...
	//channelCap, ok := k.GetCapability(ctx, host.ChannelCapabilityPath(sourcePort, sourceChannel))
	if !ok {
	...
	if err := k.ChannelKeeper.SendPacket(ctx, channelCap, packet); err != nil {
		...
		//if err := k.SendPacket(ctx, channelCap, packet); err != nil {
		return err
	}
